...
//activity completed by Len Au (z5308653) 20/07/20
...
void setup_pet(
    ...
    //end pointer for strtol conversion
    char *end_ptr;
    //the my_pet-> is the pointer to the name, the comma and 'name'
    //copies the copied name into this char
    strcpy(my_pet->name, name);
    strcpy(my_pet->type, type);
    my_pet->age = strtol(age, &end_ptr, 10);
    my_pet->weight = strtol(weight, &end_ptr, 10);
